...
add_predictor = function(data = chart, new_pred, col_name, portion = TRUE){
  if(!(col_name %in% colnames(chart))){
    ...
    data[colnames(data) == col_name] = rep(0, nrow(data))
  }
  
  for(i in 1:nrow(data)){
    data[i, colnames(data) == col_name] = str_count(tolower(data$TEXT[i]), new_pred)
    if(portion == TRUE){
      data[i, colnames(data) == col_name] = data[i, colnames(data) == col_name] / data$nword[i]
    }
  }
  return(data)
}
...
interaction_pickup_index = inter_index[,rank(inter_value) < 500]
inter_index[,rank(inter_value) == 59]
chart[,1505]
save(inter_index, file = "inter_index.RData")
save(inter_value, file = "inter_value.RData")
?save

?paste0
str(chart)
paste("STAR ~.+", paste(colnames(chart)[interaction_pickup_index[1,]],":",colnames(chart)[interaction_pickup_index[2,]], collapse = " + ", sep = ""))
as.formula(paste("STAR ~.+", paste(colnames(chart)[interaction_pickup_index[1,]],":",colnames(chart)[interaction_pickup_index[2,]], collapse = " + ", sep = "")))

colnames(chart)[interaction_pickup_index[1,]]
colnames(chart)[interaction_pickup_index[2,]]

model_withInter = lm(as.formula(paste("STAR ~.+", paste(colnames(chart)[interaction_pickup_index[1,]],":",colnames(chart)[interaction_pickup_index[2,]], collapse = " + ", sep = ""))) , data = chart[c(2,trim)])
...
resid_int = chart$STAR - yhat_int   #Calculate the residual values
...
**In this block we run basic MLR to the dataset**
```{r}
...
plot(chart$Id, resid, xlab = "ID", ylab = "Residuals", main = "Residual Plot", pch = 23,bg = "red",cex = 1.2)   #Plot the residual values versus their ID
abline(a = 0, b = 0, col = "black", lwd = 3)
sortResid = sort(abs(resid), decreasing = TRUE)   #Sort the absolute residuals in descending order
Outlier = sortResid[1:100]  #Find the outliers with the highest 100 absolute residual values
Outlier
```

**In this block we plot the histogram and the density plot of the residuals**
```{r}
par(mfrow = c(1,2))
...
inter_index = cbind(c(59, 35), c(42, 651), c(420, 1344))
inter_index
as.formula(paste("STAR ~", paste(colnames(chart)[inter_index[1,]], ":", colnames(chart)[inter_index[2,]], collapse = "+")))


colnames(chart)[inter_index[1,]]
...
test_data = read_csv("test_1519.csv")
head(test_data[(length(test_data) -40):length(test_data)])
str_count(tolower(test_data$TEXT[456]), ":\\)")
...
test_data = add_predictor(data = test_data, "1", "1")
test_data = add_predictor(data = test_data, "2", "2")
test_data = add_predictor(data = test_data, "3", "3")
test_data = add_predictor(data = test_data, "4", "4")
test_data = add_predictor(data = test_data, "5", "5")
...
write.csv(test_data, "test_1519.csv", row.names = FALSE)
test_data$PORTION = test_data$PORTION / test_data$nword
test_trim = (which(colnames(test_data) == "nchar")):length(test_data)
trim_test = (which(colnames(test_data) == "nchar")):length(test_data)   #Set "trim_test" to locate predictors after "nchar"
```

**In this block calculate the fitted star review of the test dataset**
```{r}
...
fit_data = data.matrix(test_data[test_trim]) %*% data.matrix(coefSum/count)
...
result$Expected = fit_data   #Set the second column of result as "Expected" and store the fitted value into the dataframe
