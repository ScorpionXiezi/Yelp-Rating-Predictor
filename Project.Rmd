...
for (i in 1:100){
  ...
  if(length(summary(model.random)$coefficients[-1,1]) != 503){
    print(summary(model.random))
    break
  }
  coefSum = coefSum + summary(model.random)$coefficients[-1,1]
  interSum = interSum + summary(model.random)$coefficients[1,1]
}
fitRaw = data.matrix(chart[trim]) %*% data.matrix(coefSum/100)
...
```{r}
fit_data = data.matrix(test_data[trim]) %*% data.matrix(coefSum/100)
