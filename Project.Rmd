...
write.csv(chart, "train_1520_capitalized.csv", row.names = FALSE)
...
test_data = read_csv("test_1519_capitalized.csv")
...
save(model_withInter, file = "model_withInter.RData")
```
...
yhat_bagging = data.matrix(chart[trim]) %*% data.matrix(coeff_bagging)
yhat_bagging = yhat_bagging + as.integer(intercept_bagging)

yhat_multinom = as.double(predict(model_multinom))

yhat_combine = yhat_int*0.4 + yhat_multinom*0.4 + yhat_bagging*0.2

yhat_combine[yhat_combine > 5] = 5   #Set all predicted values above 5 to 5
yhat_combine[yhat_combine < 1] = 1   #Set all predicted values below 1 to 1



resid_combine = chart$STAR_ORIGIN - yhat_combine   #Calculate the residual values
SSE_combine = sum(resid_combine^2)   #Calculate the SSE
SSE_combine
...
coeff_bagging = read.csv("coefficient.csv")/102
intercept_bagging = read.csv("intercept.csv")/102

fit_bagging = data.matrix(test_data[trim_test]) %*% data.matrix(coeff_bagging)
fit_bagging = fit_bagging + as.integer(intercept_bagging)
...
test_data_lower = read_csv("test_1519.csv")
colnames(test_data_lower)[colnames(test_data_lower) == "TRUE"] = "true"
fit_multinom = as.double(predict(model_multinom, newdata = test_data_lower[,trim_test]))

#interaction model
fit_int = predict(model_withInter, test_data[,trim_test])
...
result$Expected = fit_int*0.4+fit_multinom*0.4+fit_bagging*0.2   #Set the second column of result as "Expected" and store the fitted value into the dataframe
result$Expected
