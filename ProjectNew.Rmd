...
yhatOriginal = predict(modelOriginal)   #Calculate predicted yhat value using the prediction model
...
```{r}
par(mfrow = c(1,2))
hist(residOriginal, breaks = 100, main="Histogram of Residuals", xlab = "Residuals")   #Plot histogram
plot(density(residOriginal), main="Kernel Density of Residuals", xlab = "Residuals")   #Plot density graph
```
...
add_predictor = function(data = chart, new_pred, col_name, portion = TRUE){
  if(col_name %in% colnames(chart)){   #If the predictor exists, return the original dataset
    return(data)
  }
  data$new_col = rep(0, nrow(data))
  colnames(data)[ncol(data)] = col_name   #Initiate the new predictor
  for(i in 1:nrow(data)){
    data[i, ncol(data)] = str_count(tolower(data$TEXT[i]), new_pred)   #Count the number of that predictor
    if(portion == TRUE){
      data[i, ncol(data)] = data[i, ncol(data)] / data$nword[i]   #Get the ratio of that count to number of words
    }
  }
  return(data)
}
...
colnames(chart) = toupper(colnames(chart))   #Capitalize call column names

chart = read_csv("train_1520_capitalized.csv")
...
```{r}
model1 = lm(STAR_ORIGIN ~ . , data = chart[c(2,trim)])   #Run MLR of all predictors with STAR_ORIGIN
...
resid = chart$STAR_ORIGIN - yhat   #Calculate the residual values
...
for(i in ((which(colnames(chart) == "NWORD")) +1):length(chart)){
  if(i %% 20 == 0){   #In each 20 lines, print the value of i to keep track of the process
    print(i)
  }
  for(j in (i+1):length(chart)){
    inter = cbind(chart[2], chart[,i], chart[,j])   #Include STAR_origin, ith and jth column as the dataframe for interation
    model_inter = lm(STAR_ORIGIN ~.^2, data = inter)   #Model interation of ith, jth predictor versus STAR_origin
    if(nrow(summary(model_inter)$coefficients) == 4){   #Test if interation exists
      if(summary(model_inter)$coefficients[4,4] < 0.00000001){   #Test if interattion is significant
        inter_index = cbind(inter_index, c(i,j))   #Store the index of the significant interation predictors
        inter_value = c(inter_value, summary(model_inter)$coefficients[4,4])   #Store the p-value of the significant interation predictors
      }
    }
  }
}

model_withInter = lm(as.formula(paste("STAR_ORIGIN ~.+", paste(colnames(chart)[inter_index[1,]],":",colnames(chart)[inter_index[2,]], collapse = " + ", sep = ""))) , data = chart[c(2,trim)])   #Run interation regression based on the chosen predictors
...
for (i in 1:200){   #We perform bagging 200 times
  ...
  model.random = lm(STAR_ORIGIN ~ ., data = chart[list, c(2, trim)])   #We perform MLR on the sample
...
residNew = chart$STAR_ORIGIN - fitRaw   #Calculate residuals after bagging
...
```{r}
model_multinom = multinom(STAR_ORIGIN ~ . , data = chart[c(2,trim)], MaxNWts = 10000)   #We perform logistic regression on the sample
...
resid_multi = chart$STAR_ORIGIN - yhatMulti   #Calculate residuals of the logistic model
...
colnames(test_data)[colnames(test_data) == "PORTION"] = "PORTION_origin"
colnames(test_data)[colnames(test_data) == "NAME"] = "NAME_origin"
colnames(test_data)[colnames(chart) == "CITY"] = "CITY_origin"
colnames(test_data)[colnames(chart) == "TEXT"] = "TEXT_origin"
colnames(test_data)[colnames(test_data) == "STAR"] = "STAR_origin"
colnames(test_data)[colnames(test_data) == "Id"] = "ID_ORIGIN"
...
fit_bagging = data.matrix(test_data[trim_test]) %*% data.matrix(coefSum/count)   ##Multiply the average of the sum of each model's coefficients to the number of each predictor value counted in the test_data dataset and store them into fit_data
fit_bagging = fit_bagging + interSum/count   #Add the average of sum of each model's interceps to fit_data
fit_bagging = fit_bagging[,1]   #Convert the fitted data after bagging to a vector
```
...
superPredict = fit_int*0.4+fit_multinom*0.4+fit_bagging*0.2   #Combine all fitted values
superPredict[superPredict > 5] = 5   #Set all values greater than 5 to 5
superPredict[superPredict < 1] = 1   #Set all values less than 1 to 1
```
...
```{r}
library(ggpubr)
SSE_chart = as.data.frame(c(44077, 60084, 40601, 33853, 33389, 26014, 12006, 16859))
colnames(SSE_chart) = c("value")
SSE_chart$name = c("MLR", "multinomial logistic", "with ?!. & divided by nword", "1514 predictors", "1514 bagging", "1514 logistic & bagging", "1514 logistic & interaction", "1514 logistic, interaction & bagging")
SSE_chart$name
?ggbarplot
ggbarplot(SSE_chart, x = "name", y = "value",
          fill = "lightgray", 
          xlab = "Model", ylab = "SSE",
          ylim = c(0, 70000),
          x.text.angle = 45,
